...
mod tests {
    ...
    fn test_bool_text() {
        ...
        assert_eq!(bool::from_sql(&ty, Some(b"f"), Format::Text).unwrap(), false);
        assert_eq!(bool::from_sql(&ty, Some(b"true"), Format::Text).unwrap(), true);
        assert_eq!(bool::from_sql(&ty, Some(b"FALSE"), Format::Text).unwrap(), false);
    }

    #[test]
    fn test_bool_binary() {
        let ty = Type::BOOL;
        assert_eq!(bool::from_sql(&ty, Some(&[1]), Format::Binary).unwrap(), true);
        assert_eq!(bool::from_sql(&ty, Some(&[0]), Format::Binary).unwrap(), false);
    }
    ...
    fn test_i32_text() {
        ...
        assert_eq!(i32::from_sql(&ty, Some(b"2147483647"), Format::Text).unwrap(), i32::MAX);
    }
    ...
    fn test_i64_text() {
        let ty = Type::INT8;
        assert_eq!(i64::from_sql(&ty, Some(b"9223372036854775807"), Format::Text).unwrap(), i64::MAX);
    }
    ...
    fn test_string_to_sql() {
        let ty = Type::TEXT;
        let mut buf = Vec::new();
        "hello".to_string().to_sql(&ty, &mut buf, Format::Text).unwrap();
    ...
    fn test_vec_u8_to_sql_text_hex() {
        let ty = Type::BYTEA;
        let mut buf = Vec::new();
        vec![0xDEu8, 0xAD, 0xBE, 0xEF].to_sql(&ty, &mut buf, Format::Text).unwrap();
    ...
    fn test_vec_u8_to_sql_binary() {
        let ty = Type::BYTEA;
        let mut buf = Vec::new();
        vec![1u8, 2, 3].to_sql(&ty, &mut buf, Format::Binary).unwrap();
        assert_eq!(&buf, &[1, 2, 3]);
    }
